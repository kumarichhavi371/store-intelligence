...
class EventEmitter:
    ...
    def emit(self, partial_event: dict) -> dict:
        ...
        event = {
            ...
            "visitor_id": partial_event.get("visitor_id", f"VIS_{uuid.uuid4().hex[:6]}"),
            "event_type": partial_event.get("event_type", "ZONE_ENTER"),
            "timestamp":  partial_event.get("timestamp",
                            datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")),
            "zone_id":    partial_event.get("zone_id"),
            "dwell_ms":   partial_event.get("dwell_ms", 0),
            "is_staff":   partial_event.get("is_staff", False),
            "confidence": round(partial_event.get("confidence", 0.9), 4),
            "metadata": {
                "queue_depth": partial_event.get("metadata", {}).get("queue_depth"),
                "sku_zone":    partial_event.get("metadata", {}).get("sku_zone"),
                "session_seq": partial_event.get("metadata", {}).get("session_seq", 0),
            }
        }
